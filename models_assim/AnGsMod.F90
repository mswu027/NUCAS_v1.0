...
subroutine Vcmax_Jmax(lai_o, clumping, Vcmax0, VJ_slope,VN_slope, leaf_N, CosZs,  &
                      Vcmax_sunlit, Vcmax_shaded, Jmax_sunlit, Jmax_shaded)
implicit none

    real(r8)  :: lai_o,clumping,Vcmax0,VN_slope
    ...
    if (CosZs>0)then
        ...
        if(expr1>0)then
            Vcmax_sunlit = Vcmax0 * VN_slope * leaf_N * K*expr2 /(Kn + K) / expr1 !slope_Vcmax_N
        else
            Vcmax_sunlit = Vcmax0
        end if
        ...
        if ((K > 0).and.(lai_o > expr1/K))then
            Vcmax_shaded = Vcmax0 * VN_slope * leaf_N * (expr3 / Kn - expr2 / (Kn + K)) / (lai_o - expr1/K) !slope_Vcmax_N
        else
            Vcmax_shaded = Vcmax0
        end if
    ...
    Jmax_sunlit = Vcmax_sunlit * VJ_slope - 14.2    !vj_slope default 2.39
    Jmax_shaded = Vcmax_shaded * VJ_slope - 14.2
...
subroutine photosynthesis(LC,temp_leaf_p,f_leaf,rad_leaf,e_air,&
