...
real(r8)             :: secs_elapsed,secs_meteo,days_lai
...
if (meteo_input >= 0) then  ! call hourly meteo. input
    if (nscale == 0) then
        ...
        call timemgr_diff_secs(2007*10000+1*100+1,0,yr*10000+mn*100+dy,tod,&
            secs_meteo)
...
if (lai_input >=0) then
   call timemgr_datediff(2007*10000+1*100+1,0,yr*10000+mn*100+dy,tod,&
            days_lai)
   n_lai = int(days_lai)
      ...
      do j = 1,PFT    !! PFT iteration
           if(bfields%lcno(i,j) > 0 .and. bfields%sw(i) >0. .and. bfields%stext(i) >0 .and. bfields%clumping(i) > 0.5) then
               ...
               if(nsrest == nsrStartup .and. is_first_step()) then
                  call Init_soil_parameters(bfields%lcno(i,j),bfields%laiyr(i,j),bfields%stext(i), ppar%p_f_decay(j),&
                                            param(27),soilp)
...
!                 end do
 ...
 !              end do
               ...
               call inter_prg(yr, mn, dy, tod, &
                    lai,bfields%laiyr(i,j),lai_input,bfields%lcno(i,j),bfields%clumping(i),ppar%p_Vcmax(j),ppar%p_VJ_slope(j),&
                    ppar%p_VN_slope(j),ppar%p_f_leaf,ppar%p_sif_alpha(j),ppar%p_sif_beta(j),ppar%p_bwb(j),&
                    ppar%p_a(j),ppar%p_b(j),ppar%p_c(j),param,meteo,CosZs,&
...
!optimization purpose,@MOUSONG.WU,2019-11
               ...
               call soil_resp(ppar%p_res(j),Ccd,Cssd,Csmd,Cfsd,Cfmd,Csm,Cm,Cs,&
