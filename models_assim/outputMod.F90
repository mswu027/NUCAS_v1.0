...
contains
  ...
  subroutine write_output_global(yy,mm,dd,tod)
    ...
    PWS3    = reshape(PWS2,(/nlon,nlat/))
    ETa3    = reshape(ETa2,(/nlon,nlat/))
    ...
    if(status .ne. nf90_noerr) then
       ...
       call check(nf90_def_var(ncid,"PWS",nf90_double,(/dimid_lon,dimid_lat,dimid_time/),varid))
       ...
       call check(nf90_def_var(ncid,"ETa",nf90_double,(/dimid_lon,dimid_lat,dimid_time/),varid))
       ! call check(nf90_def_var_fill(ncid, varid,  NF90_FILL, fill_value))
       call check(nf90_put_att(ncid,varid,"missing_value",fill_value))
       call check(nf90_put_att(ncid,varid,"units","m/s"))
       call check(nf90_put_att(ncid,varid,"name","Actual transpiration"))
    ...
    call check(nf90_put_var(ncid,varid,PWS3,start=(/1,1,nt/),count=(/nlon,nlat,1/)))
    call check(nf90_inq_varid(ncid,"ETa",varid))
    call check(nf90_put_var(ncid,varid,ETa3,start=(/1,1,nt/),count=(/nlon,nlat,1/)))
